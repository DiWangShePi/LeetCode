...
fn make_problem_hits_cache(problems: &Problems) -> HashMap<String, bool> {
    ...
    for problem in &problems.problems {
        ...
        let numeric_part = id.split('-').next().unwrap_or_default();
        let numeric_value = numeric_part.parse::<i32>().unwrap_or(0);
        let match_id = numeric_value.to_string();
        result.insert(match_id, false);
    }
